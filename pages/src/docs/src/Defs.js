...
var TypeDef = React.createClass({
  render() {
    ...
    switch (type.k) {
      ...
      case TypeKind.This: return this.wrap('primitive', 'this');
      case TypeKind.Undefined: return this.wrap('primitive', 'undefined');
      ...
      case TypeKind.Union: return this.wrap('union', [
        Seq(type.types).map(t =>
          <TypeDef info={info} type={t} />
        ).interpose(' | ').toArray(),
      ]);
      case TypeKind.Tuple: return this.wrap('tuple', [
        '[',
        Seq(type.types).map(t =>
          <TypeDef info={info} type={t} />
        ).interpose(', ').toArray(),
      ...
      case TypeKind.Indexed: return this.wrap('indexed', [
        <TypeDef info={info} type={type.type} />,
        '[',
        <TypeDef info={info} type={type.index} />,
        ']'
      ]);
      case TypeKind.Operator: return this.wrap('operator', [
...
function typeLength(info, type) {
  ...
  switch (type.k) {
    ...
    case TypeKind.This: return 4;
    case TypeKind.Undefined: return 9;
    ...
    case TypeKind.Tuple: return (
      2 +
      type.types.reduce(t => typeLength(info, t)) +
      (type.types.length - 1) * 2
    );
    case TypeKind.Object: return 2 + memberLength(info, type.members);
    case TypeKind.Indexed: return 2 + typeLength(info, type.type) + typeLength(info, type.index);
    case TypeKind.Operator: return 1 + type.operator.length + typeLength(info, type.type);
    case TypeKind.Array: return typeLength(info, type.type) + 2;
    case TypeKind.Function: return 2 + funcLength(info, type);
    case TypeKind.Param: return (
      info && info.propMap[info.defining + '<' + type.param] ?
