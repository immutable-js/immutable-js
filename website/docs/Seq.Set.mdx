...
Because <CodeLink to="./Seq" /> are often lazy, `Seq.Set` does not provide the same guarantee
of value uniqueness as the concrete <CodeLink to="./Set" />.
...
<Signature
  code={`flatMap<M>(mapper: (value: T, key: T, iter: this) => Iterable<M>, context?: unknown): Seq.Set<M>`}
/>
...
<Signature
  code={`filter(predicate: (value: T, key: T, iter: this) => boolean, context?: unknown): Seq.Set<T>`}
/>
...
<Signature
  code={`filterNot(predicate: (value: T, key: T, iter: this) => boolean, context?: unknown): Seq.Set<T>`}
/>
...
<Signature
  code={`partition(predicate: (this: C, value: T, key: T, iter: this) => boolean, context?: C): [Seq.Set<T>, Seq.Set<T>]`}
/>
...
<Signature
  code={`sortBy<C>(comparatorValueMapper: (value: T, key: T, iter: this) => C, comparator?: Comparator<C>): this & OrderedSet<T>`}
/>

<Repl
  defaultValue={`Seq.Set([
    { name: 'Bob', age: 30 },
    { name: 'Max', age: 25 },
    { name: 'Lili', age: 35 }
...
<Signature
  code={`concat<C>(...valuesOrCollections: Array<Iterable<C> | C>): Seq.Set<T | C>`}
/>
