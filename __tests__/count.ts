...
describe('count', () => {
  ...
  it('counts sequences with known lengths', () => {
    expect(I.Sequence.of(1,2,3,4,5).length).toBe(5);
    expect(I.Sequence.of(1,2,3,4,5).count()).toBe(5);
  })

  it('counts sequences with unknown lengths, resulting in a cached length', () => {
    var seq = I.Sequence.of(1,2,3,4,5,6).filter(x => x % 2 === 0);
  ...
  it('counts sequences with a specific predicate', () => {
    var seq = I.Sequence.of(1,2,3,4,5,6);
  ...
  it('counts by indexed sequence', () => {
    expect(
      I.Sequence.of(1,2,3,4,5,6).countBy(x => x % 2).toJS()
    ).toEqual(
      {1:3, 0:3}
    );
  })
  ...
  it('counts by specific keys', () => {
    expect(
      I.Sequence.of(1,2,3,4,5,6).countBy(x => x % 2 ? 'odd' : 'even').toJS()
    ).toEqual(
      {odd:3, even:3}
    );
  })
