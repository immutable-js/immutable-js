...
describe('merge', () => {
  ...
  it('merge JS Objects with Symbol keys', () => {
    const c = Symbol('c');
    const d = Symbol('d');
    const e = Symbol('e');
    expect(
        merge({[c]: 1, [d]: 2 }, {[c]: 10, [e]: 20}),
    ).toEqual(
        {[c]: 10, [d]: 2, [e]: 20},
    );
  });

  it('mergeDeep JS Objects with Symbol keys', () => {
    const a = Symbol('a');
    const b = Symbol('b');
    const c = Symbol('c');
    const d = Symbol('d');
    ...
    const g = Symbol('g');
    expect(
        mergeDeep({[a]: {[b]: {[c]: 1, [d]: 2}}}, {[a]: {[b]: {[c]: 10, [e]: 20}, [f]: 30}, [g]: 40}),
    ).toEqual(
      {[a]: {[b]: {[c]: 10, [d]: 2, [e]: 20}, [f]: 30}, [g]: 40},
    );
  });

  it('merge Maps with Symbol keys', () => {
      const c = Symbol('c');
      const d = Symbol('d');
      const e = Symbol('e');
      expect(
          merge(Map({[c]: 1, [d]: 2 }), Map({[c]: 10, [e]: 20})),
      ).toEqual(
          Map({[c]: 10, [d]: 2, [e]: 20}),
      );
  });

  it('mergeDeep Maps with Symbol keys', () => {
    const a = Symbol('a');
    const b = Symbol('b');
    const c = Symbol('c');
    const d = Symbol('d');
    ...
    const expected = Map({[a]: {[b]: {[c]: 10, [d]: 2, [e]: 20}, [f]: 30}, [g]: 40});
